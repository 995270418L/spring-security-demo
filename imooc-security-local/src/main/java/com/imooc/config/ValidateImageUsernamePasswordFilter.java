...
public class ValidateImageUsernamePasswordFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        // 判断是否来自登陆请求
        if(request.getRequestURL().toString().endsWith("/login") && request.getMethod().equals("POST")){
            try{
                validate(request);
    ...
    private void validate(HttpServletRequest request) throws ServletRequestBindingException, ValidateException {
        ...
        // 判断是否超过了过期时间
        if(imageCode.getExpireTime().compareTo(LocalDateTime.now()) < 0){
        ...
        if(!formCode.equals(imageCode.getCode())){
